odoo.define("delivery_purolator.checkout", function (require) {
  ...
  publicWidget.registry.websiteSaleDelivery = publicWidget.Widget.extend({
    ...
    start: function () {
      ...
      if (showhideink) {
        showhideink.src =
          "/delivery_purolator/static/src/images/ln-plus.png.png";
      }
    ...
    _handleCarrierUpdateResult: function (result) {
      ...
      var services_name = document.getElementById("services_name");
      var purolator_table = document.getElementById("purolator_table");
      var table_load = document.getElementById("table_load");
      ...
      if (result.status === true && result.purolator_service_rates.length > 0) {
        ...
        var purolator_service_rates = rates.filter(
          (rec) =>
            rec.service_id === "PurolatorExpress" ||
            rec.service_id === "PurolatorExpressU.S." ||
            rec.service_id === "PurolatorExpressInternational" ||
            rec.service_id === "PurolatorGround"
        );

        var minRate = 0,
          services_id_value = false;
        var rate_array = [];
        ...
        function popoulate_table(datas) {
          ...
          datas.forEach(function (rec, index) {
            ...
            if (
              rec.service_id.slice(Math.max(rec.service_id.length - 2, 0)) ==
                "PM" ||
              rec.service_id.slice(Math.max(rec.service_id.length - 2, 0)) ==
                "AM"
            ) {
              var expected_time = data_arr[data_arr.length - 1].replace(
                /([0-9])([A-Z])/g,
                "$1 $2"
              );
            } else {
        ...
        for (var index = 1; index < tableRef.childElementCount; index++) {
          if (
            tableRef.children[index].children[2].innerText.replace(/\s/g, "") ==
            minRate
          ) {
            tableRef.children[
              index
            ].children[4].children[0].children[0].click();
          }
        }
        if (purolator_service_rates.length > 0) {
          purolator_table.style.display = "block";
    ...
    _onServiceClick: function (ev) {
      ...
      var service_id = selectElement.value;
      var $carriers = $('#delivery_carrier input[name="delivery_type"]');
    ...
    _onCarrierClick: function (ev) {
      ...
      //   $payButton.data(
      //     "disabled_reasons",
      //     $payButton.data("disabled_reasons") || {}
      //   );
      disabledReasons.carrier_selection = true;
      $payButton.data("disabled_reasons", disabledReasons);
    ...
    _onshowHideLink: function (ev) {
      ...
      if (src[src.length - 1] == "ln-plus.png") {
        ev.currentTarget.firstElementChild.src =
          "/delivery_purolator/static/src/images/ln-minus.png";
        ev.currentTarget.parentElement.parentElement.parentElement.children[2].children[0].firstElementChild.style.display =
          "block";
      } else if (src[src.length - 1] == "ln-minus.png") {
        ev.currentTarget.firstElementChild.src =
          "/delivery_purolator/static/src/images/ln-plus.png";
        ev.currentTarget.parentElement.parentElement.parentElement.children[2].children[0].firstElementChild.style.display =
          "none";
      }
    },
